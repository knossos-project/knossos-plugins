...
def _create_qApp():
    ...
    """
    if QT_API == QT_API_PYTHONQT5:  # we already have an application running
        return
    global qApp
    if qApp is None:
        if DEBUG:
...
class MainWindow(QtWidgets.QMainWindow):
    closing = QtCore.Signal() if QT_API == QT_API_PYQT5 else Signal()
...
class FigureManagerQT(FigureManagerBase):
    ...
    except AttributeError:  # PythonQt5
        def _show_message(self, s):
            # Fixes a PySide segfault.
            self.window.statusBar().showMessage(s)
...
class NavigationToolbar2QT(NavigationToolbar2, QtWidgets.QToolBar):
    message = QtCore.Signal(str) if QT_API == QT_API_PYQT5 else Signal()
